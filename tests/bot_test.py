...
# アクセストークン
TOKEN = "1508945228:AAETVoadpXvDw8Z5yX0jVLqK4SlDeROQtNQ"

# 1対話の長さ(ユーザの発話回数)．ここは固定とする
DIALOGUE_LENGTH = 15

#現在のシナリオ(ここではスロットと呼んでいる)
TOPIC_NAME = "kobayashi_only"

#現在のルールの書かれたパス
RULE_PASS = "rule/rule_k.json"
...
def reply(context, generator):
    return generator.reply(context)
...
class SampleBot:
    ...
    def start(self, bot, update):
        # 対話ログと発話回数を初期化
        self.user_context[update.message.from_user.id] = {"context": [], "count": 0, "generator": Generator(TOPIC_NAME, RULE_PASS)}

        # システムからの最初の発話
        # 以下の発話に限定しません．任意の発話を返してください
        update.message.reply_text('小林と言ってください')
    ...
    def message(self, bot, update):
        ...
        send_message = reply(self.user_context[update.message.from_user.id]["context"], self.user_context[update.message.from_user.id]["generator"])
